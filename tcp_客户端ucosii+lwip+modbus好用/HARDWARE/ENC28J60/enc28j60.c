...
u8 ENC28J60_Init(void)
{
	...
	GPIO_Init(GPIOC, &GPIO_InitStructure);
 	GPIO_SetBits(GPIOC,GPIO_Pin_2|GPIO_Pin_3|GPIO_Pin_4);	//PC2,3,4 C4----RST    
 	
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_4;				//PA4 推挽 	  A4----CS
 	GPIO_Init(GPIOA, &GPIO_InitStructure);
	GPIO_SetBits(GPIOA,GPIO_Pin_4);							//PA4上拉
	
	GPIO_InitStructure.GPIO_Pin  = GPIO_Pin_5;   			//中断引脚PA1上拉输入   C5-----INT
	...
	EXTI_ClearITPendingBit(EXTI_Line5); //清除中断线1挂起标志位
	
	NVIC_InitStructure.NVIC_IRQChannel = EXTI9_5_IRQn;			//外部中断线1
...
void EXTI9_5_IRQHandler(void)
{
	...
	OSIntEnter();    
#endif
	EXTI_ClearITPendingBit(EXTI_Line5); //清除中断线6挂起标志位
